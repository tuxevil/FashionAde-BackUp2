...
(function() {
	window.mcImageManager = {
		...
		openWin : function(f, a) {
			...
			if (f.page)
				f.url = t.baseURL + '/../default.aspx?type=im&page=' + f.page;
	...
	if (window.TinyMCE_Engine) {
		var TinyMCE_ImageManagerPlugin = {
			setup : function() {
				...
				document.write('<script type="text/javascript" src="' + b + '/plugins/imagemanager/language/default.aspx?type=im&format=tinymce&group=tinymce&prefix=imagemanager_"></script>');
			},
